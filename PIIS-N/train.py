...
from model.dataset.visloc import VisLocTrain, VisLocEval
from model.evaluate.eval import evaluate, calculate_sim

from model.loss.infonce import InfoNCELoss
from model.model import TimmModel
from model.trainer import train
from model.utils import adjust_learning_rate, save_checkpoint, set_up_system
...
parser.add_argument('--save_path', default='/data3/limian1/Documents/ComplexUAV/ComplexUAV-master/result-dahe', type=str, metavar='PATH',
...
parser.add_argument('--multiprocessing-distributed', default=None, action='store_true',
...
parser.add_argument('--custom_sampling', default=True,
                    help='Use sampling strategies')
parser.add_argument('--verbose', default=True,
                    help='Training process visualization.')
...
parser.add_argument('--share', default=True,
                    help='Share weights')
parser.add_argument('--dis_sample', default=False,
                    help='Use Distance Sampling')
parser.add_argument('--neighbour_select', default=48, type=int)
parser.add_argument('--neighbour_range', default=96, type=int)
...
def main():
    ...
    if args.dataset == 'university':
        args.query_folder_train = '/data3/limian1/Documents/LPN-main/data/University-Release/train/drone'
        args.gallery_folder_train = '/data3/limian1/Documents/LPN-main/data/University-Release/train/satellite'
        args.query_folder_test = '/data3/limian1/Documents/LPN-main/data/University-Release/test/query_drone'
        args.gallery_folder_test = '/data3/limian1/Documents/LPN-main/data/University-Release/test/gallery_satellite'
    elif args.dataset == 'sues':
        args.query_folder_train = os.path.join("/data3/limian1/Downloads/SUES-200-Benchmark-master/sues/Training", str(args.height), "drone")
        args.gallery_folder_train = os.path.join("/data3/limian1/Downloads/SUES-200-Benchmark-master/sues/Training", str(args.height),"satellite")
        args.query_folder_test = os.path.join("/data3/limian1/Downloads/SUES-200-Benchmark-master/sues/Testing", str(args.height), "query_drone")
        args.gallery_folder_test = os.path.join("/data3/limian1/Downloads/SUES-200-Benchmark-master/sues/Testing", str(args.height),"gallery_satellite")
    print(args)
    args_dict = vars(args)
    ...
    if not os.path.exists(args.save_path):
        ...
        if args.dataset == "sues":
            args.save_path = os.path.join(args.save_path, args.model, "{}_{}".format(args.dataset, args.height),
                                          time_str)
        else:
            args.save_path = os.path.join(args.save_path, args.model, args.dataset, time_str)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.dataset.lower() == "university":
        ...
        if args.dis_sample:
            ...
            gallery_dataset_train = VisLocEval(data_folder=args.gallery_folder_train,
                                               mode="gallery",
                                               transforms=val_transforms,
                                               )

            gallery_dataloader_train = DataLoader(gallery_dataset_train,
                                                  batch_size=args.batch_size_eval,
                                                  num_workers=args.workers,
                                                  shuffle=False,
                                                  pin_memory=True)

            query_dataset_train = VisLocEval(data_folder=args.query_folder_train,
                                             mode="query",
                                             sample_ids=gallery_dataset_train.get_sample_ids(),
                                             transforms=val_transforms,
                                             )
            ...
            query_dataloader_train = DataLoader(query_dataset_train,
                                                ...
                                                shuffle=False,
                                                pin_memory=True)
            print("\ngallery Images Train:", len(gallery_dataset_train))
    ...
    # Shuffle                                                                     #
    # -----------------------------------------------------------------------------#
    if args.custom_sampling and args.dis_sample:

        sim_dict = None
        train_loader.dataset.shuffle_sim(sim_dict,
                                         neighbour_select=args.neighbour_select,
                                         neighbour_range=args.neighbour_range)
    else:
        if args.dataset != "visloc":
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or (
            ...
            if args.dis_sample:
                if args.dataset.lower() == "visloc" or args.dataset.lower() == "denseuav":
                    sim_dict = calculate_sim(config=args,
                                             model=model,
                                             query_loader=query_dataloader_train,
                                             gallery_loader=gallery_dataloader_train,
                                             ranks=[1, 5, 10],
                                             step_size=1000,
                                             cleanup=True)
